...
<main class="fp-hero-main container">
		...
		foreach ($tabs_config as $tab) {
			...
			if ($q->have_posts()) {
				while ($q->have_posts()) {
					...
					$dummy_img = get_post_meta(get_the_ID(), '_tcc_dummy_image', true) ?: 'https://images.unsplash.com/photo-1600607686527-6fb886090705?auto=format&fit=crop&q=80&w=400';
	...
	<section class="subscribe-section">
		<div>
			<h3 class="text-serif">subscribe</h3>
			<p class="text-sans">Sign up for weekly style inspiration right to your inbox.</p>
		</div>
		<div class="flex subscribe-input-group">
			<input type="email" placeholder="Email Address" />
			<button class="uppercase text-sans">Subscribe</button>
		</div>
	</section>
