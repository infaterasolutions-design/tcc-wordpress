...
function tcc_theme_setup() {
    ...
    // --- TEMPORARY FEED LOCATOR BYPASS ---
    if (isset($_GET['find_feed'])) {
        global $wpdb;
        $log = "--- Searching Posts/Pages ---\n";
        $posts = $wpdb->get_results("SELECT ID, post_title, post_type FROM $wpdb->posts WHERE post_content LIKE '%[instagram-feed]%' OR post_content LIKE '%[smash_balloon]%' OR post_content LIKE '%wp-block-sbi-sbi-feed-block%'");
        foreach ($posts as $p) {
            $log .= "Found in " . $p->post_type . " ID: " . $p->ID . " - " . $p->post_title . "\n";
        }
        $log .= "\n--- Searching Widgets ---\n";
        $widgets = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'widget_%' AND option_value LIKE '%instagram%'");
        foreach ($widgets as $w) {
            $log .= "Found in widget: " . $w->option_name . "\n";
        }
        $log .= "\n--- Searching Theme Mods ---\n";
        $theme_mods = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'theme_mods_%' AND option_value LIKE '%instagram%'");
        add_action('admin_notices', function() {
            ...
            $log .= "\n--- Searching Theme Mods ---\n";
            $theme_mods = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'theme_mods_%' AND option_value LIKE '%instagram%'");
            foreach ($theme_mods as $tm) {
                $log .= "Found in theme mod: " . $tm->option_name . "\n";
            }
            echo "<div class='notice notice-info'><pre>" . esc_html($log) . "</pre></div>";
        });
    }
    // -------------------------------------
...
// --- TEMPORARY FEED LOCATOR (ADMIN DASHBOARD) ---
add_action( 'admin_notices', 'tcc_locate_instagram_feed' );
function tcc_locate_instagram_feed() {
    global $wpdb;
    $log = "<strong>TCC Feed Locator Results:</strong><br>";
    
    $posts = $wpdb->get_results("SELECT ID, post_title, post_type FROM $wpdb->posts WHERE post_content LIKE '%[instagram-feed]%' OR post_content LIKE '%[smash_balloon]%' OR post_content LIKE '%wp-block-sbi-sbi-feed-block%'");
    foreach ($posts as $p) {
        $log .= "Found in " . esc_html($p->post_type) . " (ID: " . esc_html($p->ID) . ") - " . esc_html($p->post_title) . "<br>";
    }
    
    $widgets = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'widget_%' AND option_value LIKE '%instagram%'");
    foreach ($widgets as $w) {
        $log .= "Found in widget: " . esc_html($w->option_name) . "<br>";
    }
    
    $theme_mods = $wpdb->get_results("SELECT option_name FROM $wpdb->options WHERE option_name LIKE 'theme_mods_%' AND option_value LIKE '%instagram%'");
    foreach ($theme_mods as $tm) {
        $log .= "Found in theme mod: " . esc_html($tm->option_name) . "<br>";
    }
    
    if ($log === "<strong>TCC Feed Locator Results:</strong><br>") {
        $log .= "No Smash Balloon feeds were found in the database.";
    }
    
    echo '<div class="notice notice-success is-dismissible"><p>' . $log . '</p></div>';
}
// ------------------------------------------------

/**
 * DEFER SMASH BALLOON JAVASCRIPT
 * This prevents the Smash Balloon plugin from slowing down the website's initial page load.
 */
add_filter( 'script_loader_tag', 'tcc_defer_smash_balloon_js', 10, 3 );
function tcc_defer_smash_balloon_js( $tag, $handle, $src ) {
    if ( strpos( $handle, 'sbi_scripts' ) !== false ) {
...
/**
 * NATIVE DOM SCRUBBER: Automatically clean up messy HTML generated by Google Sheet automations.
 * This uses PHP's native DOMDocument to safely and flawlessly remove Google's hidden inline styles,
 * <span> wrappers, and empty paragraphs without relying on fragile Regex.
 */
add_filter( 'wp_insert_post_data', 'tcc_clean_automation_html_dom', 10, 2 );
function tcc_clean_automation_html_dom( $data, $postarr ) {
    if ( $data['post_type'] !== 'post' ) return $data;
    
    $content = wp_unslash($data['post_content']);
    if (empty(trim($content))) return $data;

    // Suppress DOM warnings for slightly malformed HTML
    libxml_use_internal_errors(true);
    $dom = new DOMDocument();
    
    // Load HTML (using a wrapper to ensure UTF-8 handling)
    ...
    foreach ($imgs as $img) {
        ...
        while ($img->attributes->length > 0) {
            $img->removeAttribute($img->attributes->item(0)->name);
        }
    ...
    // (This fixes the massive width issue where Gutenberg inherits Google's span styles)
    $spans = $xpath->query('//span[.//img]');
    foreach ($spans as $span) {
        $span->removeAttribute('style');
        $span->removeAttribute('class');
    }
    $ps = $xpath->query('//p[.//img]');
    foreach ($ps as $p) {
        $p->removeAttribute('style');
        $p->removeAttribute('class');
    }
    
    // 3. Completely remove empty paragraphs (fixes the massive gap issue)
    $all_ps = $xpath->query('//p');
    foreach ($all_ps as $p) {
        // Strip normal spaces, non-breaking spaces, and unicode empty characters
        $textContent = trim(str_replace(["\xc2\xa0", "&nbsp;", "&#160;"], '', $p->textContent));
        // If the paragraph has no text AND no images inside it, delete it!
        if ($textContent === '' && $p->getElementsByTagName('img')->length === 0) {
            $p->parentNode->removeChild($p);
        }
    }
    
    // 4. Remove empty spans
    $all_spans = $xpath->query('//span');
    foreach ($all_spans as $span) {
        $textContent = trim(str_replace(["\xc2\xa0", "&nbsp;", "&#160;"], '', $span->textContent));
        if ($textContent === '' && $span->getElementsByTagName('img')->length === 0) {
            $span->parentNode->removeChild($span);
        }
    }

    // Rebuild the clean HTML string
    $clean_content = '';
...
function tcc_render_instagram_feed( $atts ) {
    ...
    if ( false === $feed_html ) {
        ...
        // Generate clean, semantic HTML with CSS Grid (Seamless 6-column edge-to-edge layout)
        $feed_html = '<div class="tcc-instagram-grid" style="display: grid; grid-template-columns: repeat(6, 1fr); gap: 0; margin: 0; width: 100%;">';
        ...
        foreach ( $data['data'] as $post ) {
            ...
            // Wrap in hover effect container (No border radius, seamless edges)
            $feed_html .= '<a href="' . esc_url( $permalink ) . '" target="_blank" rel="noopener nofollow" style="display: block; overflow: hidden; aspect-ratio: 1 / 1;">';
            
            // Native lazy-loading and inline object-fit
            $feed_html .= '<img src="' . esc_url( $image_url ) . '" alt="' . esc_attr( $caption ) . '" loading="lazy" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.4s ease;" onmouseover="this.style.transform=\'scale(1.05)\'" onmouseout="this.style.transform=\'scale(1)\'">';
