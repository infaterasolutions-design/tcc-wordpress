...
// Client-side Javascript cleaner to aggressively scrub raw HTML inside Classic blocks
// before the user clicks "Convert to blocks". This catches anything the backend filter missed.
    ...
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(function() {
            if (typeof wp !== 'undefined' && wp.data && wp.data.select && wp.data.dispatch) {
                // We only want to clean it once per load to prevent infinite loops
                ...
                wp.data.subscribe(function() {
                    ...
                    if (blocks && blocks.length > 0) {
                        ...
                        blocks.forEach(block => {
                            // Target the raw HTML generated by automations (Classic block)
                            if (block.name === 'core/freeform' && block.attributes.content) {
                                let html = block.attributes.content;
                                let originalHtml = html;
                                
                                // 1. Strip ALL width, height, and style attributes from images
                                // We run it multiple times to catch multiple attributes
                                for (let i = 0; i < 4; i++) {
                                    html = html.replace(/(<img[^>]+?)\s+(?:width|height|style)=["'][^"']*["']/gi, '$1');
                                }
                                
                                // 2. Strip hidden Google Docs spans and non-breaking spaces
                                html = html.replace(/<span[^>]*>(\s|&nbsp;|&#160;|<br\s*\/?>)*<\/span>/gi, '');
                                
                                // 3. Strip empty paragraphs
                                html = html.replace(/<p[^>]*>(\s|&nbsp;|&#160;|<br\s*\/?>)*<\/p>/gi, '');
                                
                                // 4. Aggressively strip line breaks immediately following an image tag
                                html = html.replace(/(<img[^>]+>)(?:\s|&nbsp;|&#160;|<br\s*\/?>)+/gi, '$1');
                                
                                // If changes were made, update the block in Gutenberg memory instantly!
                                if (html !== originalHtml) {
                                    wp.data.dispatch('core/block-editor').updateBlockAttributes(block.clientId, { content: html });
                                    blocksUpdated = true;
                                }
                            }
                        });
                        ...
                        if (blocksUpdated) {
                            hasCleaned = true;
                            console.log("TCC: Successfully scrubbed automation HTML in real-time!");
                        }
                    }
                });
            }
        }, 1500); // Wait for Gutenberg to finish rendering
    });
    </script>
    <?php
});
